...
var prefix = '!';
var token = 'NTIyNDcwMDIwMjExNTM5OTY4.DvO13g.DLThGkjhzzYxugBbXNPowy5Ddg4' //lien du Bot : https://discordapp.com/oauth2/authorize?client_id=522470020211539968&scope=bot&permissions=8

client.login(token)

client.on('ready' , function (){
	client.user.setGame("me Developper")
	console.log("Je suis connecte")
})

client.on('message' , function(message){
		if(message.content.includes(prefix + 'deco')){
				message.channel.send('**__a toute :cry:__**')
...
client.on('message' , message =>{
		...
		if (message.content === prefix + "help"){
				var embed = new Discord.RichEmbed()
					   ...
					   .addField("!del","pour supprimer votre msg.", true)
					   .setColor("0xDBA901")
...
client.on('message',message =>{
    ...
    if (args[0].toLocaleLowerCase() === prefix + 'kick'){
       ...
       if (!member.kickable) return message.channel.send("Je ne peux pas exclure cet utilisateur :confused: ")
       member.kick()
       message.channel.send("**"+member.user.username + '** a ete exclu :white_check_mark:')
       console.log("Une personne en moins")
    }
});
